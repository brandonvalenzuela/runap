...
class _MapScreenState extends State<MapScreen> with WidgetsBindingObserver {
    ...
    ever(workoutController.workoutData, (callback) {
      ...
      if (wasActive && !isActive) {
        ...
        final duration = workoutController.workoutStartTime.value != null
            ? DateTime.now()
                .difference(workoutController.workoutStartTime.value!)
            : Duration.zero;
  ...
  Future<void> _initializeSimulationPosition() async {
    ...
    _lastSimulatedPosition = initialPos ??
        Position(
          latitude: 40.416775,
          longitude: -3.703790,
          timestamp: DateTime.now(),
    ...
    print(
        "🗺️ MapScreen - Initial simulation position set: ${_lastSimulatedPosition?.latitude}, ${_lastSimulatedPosition?.longitude}");
  }
    ...
    if (_isSimulating) {
      ...
      if (workoutController.workoutData.value.isWorkoutActive == false) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              "Inicia un entrenamiento para simular la ubicación.",
            ),
          ),
        );
        return;
      ...
      LatLng? currentPosition =
          workoutController.workoutData.value.currentPosition;
      if (currentPosition == null && mapController.lastKnownPosition != null) {
        currentPosition = LatLng(
          mapController.lastKnownPosition!.latitude,
          mapController.lastKnownPosition!.longitude,
        );
      }
      ...
      if (currentPosition == null) {
        print(
            "SimToggle: Last known position is null, attempting to fetch current...");
        try {
          final pos = await mapController.locationService.getCurrentPosition();
          currentPosition = LatLng(pos.latitude, pos.longitude);
          mapController.lastKnownPosition =
              pos; // Update MapController's knowledge too
        } catch (e) {
          print(
              "SimToggle: Cannot start simulation: Failed to get current position: $e");
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                "No se pudo obtener ubicación actual para simular.",
              ),
            ),
          );
          workoutController.resumeRealLocationUpdates();
      ...
      _lastSimulatedPosition = Position(
          ...
          accuracy: 5.0,
          altitude:
              workoutController.workoutData.value.previousPosition?.altitude ??
                  mapController.lastKnownPosition?.altitude ??
                  0.0,
          altitudeAccuracy: 10.0,
          heading: 0.0,
          headingAccuracy: 0.0,
      ...
      print(
          "▶️ Starting Simulation from ACTUAL: ${_lastSimulatedPosition?.latitude}, ${_lastSimulatedPosition?.longitude}");
      ...
      _simulationTimer = Timer.periodic(const Duration(seconds: 2), (timer) {
        ...
        final Position newPosition = Position(
          ...
          altitude:
              _lastSimulatedPosition!.altitude + (Random().nextDouble() - 0.5),
        ...
        workoutController.handleLocationUpdate(
      ...
      body: Obx(
        () {
          if (mapController.isLoading.value ||
              goalController.isLoadingGoals.value) {
          ...
          return Stack(
            children: [
                ...
                () => GoogleMap(
                  ...
                  initialCameraPosition: const CameraPosition(
                    target: LatLng(
                      40.416775,
                      -3.703790,
                    ), // Madrid as fallback initial center
                    zoom: 14.0,
                  ),
                  ...
                  myLocationEnabled: mapController
                          .permissionController.permissionStatus.value ==
              ...
              if (goalController.showGoalSelector.value)
                Positioned.fill(
                  child: _buildGoalSelectionOverlay(
                    dark,
                    goalController,
                  ),
                ),
  ...
  AppBar _buildAppBar(BuildContext context, GoalController goalCtrl) {
    return AppBar(
      ...
      title: Obx(() => Text(
            workoutController.sessionToUpdate.value?.workoutName ??
                'Entrenamiento',
          )),
      actions: [
        Obx(() => IconButton(
              icon: Icon(goalCtrl.showGoalSelector.value
                  ? Icons.close
                  : Icons.flag_outlined),
  ...
  Widget _buildTopControls(bool dark) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: dark ? Colors.black.withAlpha(179) : Colors.white.withAlpha(204),
      ...
      child: Row(
        ...
        children: [
          IconButton(
            onPressed: mapController.resetMapView,
  ...
  Widget _buildGoalSelectionOverlay(bool dark, GoalController goalCtrl) {
    return Container(
      color: dark ? Colors.black.withAlpha(204) : Colors.white.withAlpha(230),
                    ...
                    return ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final bool isSelected =
                            workoutController.workoutData.value.goal == goal;
                        return ListTile(
                          leading: Icon(
                              isSelected
                                  ? Icons.check_circle
                              ...
                              color: isSelected
                                  ...
                                  : Colors.grey),
                          title: Text(
                              '${goal.formattedTargetDistance} km en ${goal.targetTimeMinutes} min'),
          ...
          children: [
            Row(
              ...
              children: [
                _buildMetricDisplay(
                  "Distancia",
                  "${(workoutCtrl.workoutData.value.distanceMeters / 1000).toStringAsFixed(2)} km",
                ),
                _buildMetricDisplay(
                  "Tiempo",
                  workoutCtrl.getFormattedElapsedTime(),
                ),
                _buildMetricDisplay(
                  "Ritmo",
                  workoutCtrl.workoutData.value.getPaceFormatted(),
                ),
              ],
            ),
            const SizedBox(height: TSizes.spaceBtwItems),
            ElevatedButton(
              style: ElevatedButton.styleFrom(
                backgroundColor: workoutCtrl.workoutData.value.isWorkoutActive
                    ? Colors.red
                    : Theme.of(context).primaryColor,
              ...
              child: Text(
                workoutCtrl.workoutData.value.isWorkoutActive
                    ? 'DETENER'
                    : 'INICIAR',
  ...
  void _showCompletionDialog(WorkoutCompletionInfo info) {
    Get.dialog(
      AlertDialog(
        title: Text(info.hadGoal && info.goalAchieved
            ? '¡Objetivo Cumplido!'
        ...
        content: SingleChildScrollView(
          child: ListBody(
            children: <Widget>[
              ...
              _buildMetricRow(
                'Ritmo Prom.',
                "${info.averagePaceMinutesPerKm.toStringAsFixed(2)} min/km",
              ),
            ],
          ),
        ),
  ...
  Widget _buildCompletionMessage(WorkoutCompletionInfo info) {
    if (info.hadGoal && info.goalAchieved) {
      ...
      return const Text(
          'No alcanzaste tu objetivo, ¡pero completaste el entrenamiento!');
    } else {
